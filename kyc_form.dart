...
class _KYCFormScreenState extends State<KYCFormScreen> {
  ...
  Future<void> _extractKTPData(XFile ktpImage) async {
    setState(() => _isScanning = true);

    try {
      final inputImage = InputImage.fromFilePath(ktpImage.path);
      ...
      final Match? nikMatch = nikRegex.firstMatch(fullText);
      bool nikFound = false;
      if (nikMatch != null) {
        final String nik = nikMatch.group(0)!;
        if (mounted) {
          setState(() => _nikCtrl.text = nik);
          nikFound = true;
        }
      }
      ...
      for (int i = 0; i < lines.length; i++) {
        ...
        if (line.contains('alamat')) {
          ...
          if (mounted) {
            setState(() => _addrCtrl.text = address.trim());
            addressFound = true;
          }
          break;
        }
      ...
      // Show snackbars
      if (mounted) {
        if (nikFound && addressFound) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content:
                  Text("NIK dan Alamat berhasil ditemukan dan diisi otomatis!"),
              backgroundColor: Colors.green));
        } else if (nikFound) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text(
                  "NIK berhasil ditemukan. Alamat tidak ditemukan, silakan isi manual."),
              backgroundColor: Colors.orange));
        } else if (addressFound) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text(
                  "Alamat berhasil ditemukan. NIK tidak ditemukan, silakan isi manual."),
              backgroundColor: Colors.orange));
        } else {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content:
                  Text("NIK dan Alamat tidak ditemukan. Silakan isi manual."),
              backgroundColor: Colors.orange));
        }
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text("Error OCR: $e"), backgroundColor: Colors.red));
      }
    } finally {
  ...
  Future<void> _submitKYC() async {
    ...
    if (_selfie == null || _ktp == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text("Foto Selfie & KTP Wajib!"),
    ...
    try {
      ...
      showDialog(
          ...
          builder: (ctx) => AlertDialog(
                ...
                content: const Text(
                    "Data diri Anda telah dikirim. Admin sedang memverifikasi data Anda. Mohon bersabar..."),
                actions: [
                  ElevatedButton(
                    onPressed: () {
                      Navigator.pop(ctx);
      ...
      GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
              ...
              border:
                  Border.all(color: file == null ? Colors.grey : Colors.green),
          ...
          child: file == null
              ? Column(
                  ...
                  children: const [
                      ...
                      Text("Scan Foto", style: TextStyle(fontSize: 10))
                    ])
              : ClipRRect(
  ...
  Widget build(BuildContext context) {
    if (_isShowingCamera && _isCameraReady)
      return Scaffold(body: _buildCameraOverlay());
      ...
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(children: [
            const Text("Lengkapi data untuk verifikasi otomatis",
            ...
            TextFormField(
                controller: _nikCtrl,
                readOnly: _nikCtrl.text.isNotEmpty,
                keyboardType: TextInputType.number,
                maxLength: 16,
                inputFormatters: [FilteringTextInputFormatter.digitsOnly],
                decoration: InputDecoration(
                    labelText: _isScanning
                        ? "Sedang membaca data KTP otomatis..."
                        : "NIK (Otomatis)",
            ...
            TextFormField(
                ...
                maxLines: 2,
                decoration: const InputDecoration(
                    labelText: "Alamat Domisili", border: OutlineInputBorder()),
                validator: (v) => v!.isEmpty ? "Wajib diisi" : null),
            const SizedBox(height: 10),
            TextFormField(
                controller: _jobCtrl,
